...
class _EditJournalScreenState extends State<EditJournalScreen> {
  ...
  void initState() {
    ...
    // Initialize fields with the journal data
    _descriptionController.text = widget.journal['description'] ?? '';
    _amountController.text =
        NumberFormat("#,###.##").format(widget.journal['amount']).toString();
    _selectedAccount = widget.journal['account_id'];
    _selectedTrack = widget.journal['track_id'];
    _transactionType = widget.journal['transaction_type'] ?? 'credit';
    _currency = widget.journal['currency'] ?? 'AFN';
    _selectedDate = widget.journal['date'] != null
        ? DateTime.parse(widget.journal['date'])
        : DateTime.now();

    // Initialize track selection
    if (_selectedTrack == _treasureTrackId) {
      _selectedTrackOption = 'treasure';
    } else if (_selectedTrack == _noTreasureTrackId) {
      _selectedTrackOption = 'noTreasure';
    } else {
      _selectedTrackOption = 'track';
      _customTrackName = _accounts.firstWhere(
        (account) => account['id'] == _selectedTrack,
        orElse: () => {'name': 'Track'},
      )['name'] as String;
    }
  }
  ...
  void didChangeDependencies() {
    ...
    // Initialize Persian date field with the current Jalali date and time if not already set.
    if (_faDateController.text.isEmpty) {
      ...
      // Initialize Persian date field with the selected date
...
// Combined date and time picker method.
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text(localizations.editJournal),
        actions: [
          ElevatedButton.icon(
